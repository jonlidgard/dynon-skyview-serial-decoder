...
class Decoder:
    ...
    _D1X0_EFIS_STEP = [8,4,5,3,4,5,4,3,3,2,6,2,2,2]
    _D1X0_EMS_STEP = [8,4,3,3,3,3,3,3,3,4,3,3,8,8,8,4,4,4,4,4,4,4,3,3,3,3,3,3,1,1,2,2]
        ...
        for line in infile:
            ...
            try:
                if output_header == True:
                    print(self._output_CSV_header(line))
                    output_header = False
                print(self._output_CSV_line(line))
            except InvalidDataException:
                continue

      #  f.close()

    def _output_CSV_header(self, data: str) -> str:
    ...
    def _output_CSV_line(self, data: str) -> str:
       result = self.decode(data)
    ...
    def decode(self, data: str, conversion: ConversionType | None = 'uk') -> ADAHRSData | SystemData | EMSData | D1x0EMSData | D1x0EFISData:

        if data.startswith('!'):
            # Probably SkyView data     
            try:
                if not self._is_valid_crc(data):
            ...
            # Probably D1x0 data     
            try:
                if self._is_valid_d1x0_efis_data(data):
                    self.d1x0_efis_data = self.parse(data, D1x0DataType.EFIS)
                    return self.d1x0_efis_data
                elif self._is_valid_d1x0_ems_data(data):
                    self.d1x0_ems_data = self.parse(data, D1x0DataType.EMS)
                    return self.d1x0_ems_data
                else:
                    raise InvalidDataException
            except ValueError:
                raise InvalidDataException
    ...
    def parse(self, data: str, data_type: str) -> ADAHRSData | SystemData | EMSData | D1x0EMSData | D1x0EFISData:
        if data_type == SkyviewDataType.ADAHR:
            ...
            step = self._EMS_STEP
        elif data_type == D1x0DataType.EFIS:
            step = self._D1X0_EFIS_STEP
        elif data_type == D1x0DataType.EMS:
            step = self._D1X0_EMS_STEP
        else:
    ...
    def _post_parse(self, data: list[str], data_type: str) -> ADAHRSData | SystemData | EMSData | D1x0EMSData | D1x0EFISData:
        if data_type == SkyviewDataType.ADAHR:
            ...
            return self._map_d1x0_efis_data(data)
        elif data_type == D1x0DataType.EMS:
            return self._map_d1x0_ems_data(data)
        else:
            raise InvalidDataTypeException
    ...
    def _is_valid_crc(self, data: str) -> bool:        
        transmitted_crc = int(data[-2:],16)
        return sum([ord(c) for c in data[:-2]]) & 0xff == transmitted_crc
    

    def _is_valid_d1x0_efis_data(self, data: str) -> bool:
        return len(data) == 51 and self._is_valid_crc(data)


    def _calc_d1x0_ems_crc(self, data: str) -> int:
        l = [ord(c) for c in data[:-2]]
        return (sum(l) & 0xff)
    

    def _is_valid_d1x0_ems_data(self, data: str) -> bool:
        ems_data_len_correct = (len(data) == 119)
        if not ems_data_len_correct: 
            return False
        transmitted_crc = int(data[-2:],16)
        crc = self._calc_d1x0_ems_crc(data)
        return (crc + transmitted_crc) & 0xff == 0


    def _degF_to_degC(self, value: int) -> int:
        if self.conversion == 'uk':
            return round(int((value -32) * 5 / 9), 1)
        else:   
            return value
        
    def _usg_to_litres(self, value: int) -> int:
        if self.conversion == 'uk':
            return round(value * 3.8, 1)
        else:   
            return value

    def _map_d1x0_ems_data(self, data: list[str]) -> D1x0EMSData:
        return D1x0EMSData(
            system_time=data[0],
            manifold_pressure=self._parse_value(data[1], division=100),
            oil_temp=self._degF_to_degC(self._parse_value(data[2])),
            ...
            fuel_flow=self._usg_to_litres(self._parse_value(data[8], division=10)),
            fuel_remaining=self._usg_to_litres(self._parse_value(data[9], division=10)),
            fuel_level_1=self._usg_to_litres(self._parse_value(data[10], division=10)),
            fuel_level_2=self._usg_to_litres(self._parse_value(data[11], division=10)),
            gp_1=data[12],
            gp_2=data[13],
            gp_3=data[14],
            gp_thermocouple=self._degF_to_degC(self._parse_value(data[15])),
            egt_1=self._degF_to_degC(self._parse_value(data[16])),
            egt_2=self._degF_to_degC(self._parse_value(data[17])),
            egt_3=self._degF_to_degC(self._parse_value(data[18])),
            egt_4=self._degF_to_degC(self._parse_value(data[19])),
            egt_5=self._degF_to_degC(self._parse_value(data[20])),
            egt_6=self._degF_to_degC(self._parse_value(data[21])),
            cht_1=self._degF_to_degC(self._parse_value(data[22])),
            cht_2=self._degF_to_degC(self._parse_value(data[23])),
            cht_3=self._degF_to_degC(self._parse_value(data[24])),
            cht_4=self._degF_to_degC(self._parse_value(data[25])),
            cht_5=self._degF_to_degC(self._parse_value(data[26])),
            cht_6=self._degF_to_degC(self._parse_value(data[27])),
            contact_1=self._parse_value(data[28]),
            contact_2=self._parse_value(data[29]),
        )
    
    def _map_d1x0_efis_data(self, data: list[str]) -> D1x0EFISData:
        return D1x0EFISData(
            system_time=data[0],
